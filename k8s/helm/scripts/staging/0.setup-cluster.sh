#!/bin/bash
# =============================================================================
# Kind 클러스터 + Istio Ambient 설정 (staging 환경)
# =============================================================================
# - 레지스트리: AWS ECR
...
echo "🚀 Kind 클러스터 + Istio Ambient 설정 (staging - AWS ECR)"
...
if grep -q "<AWS_ACCOUNT_ID>" "${STAGING_YAML}" 2>/dev/null; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sed -i '' "s/<AWS_ACCOUNT_ID>/${AWS_ACCOUNT_ID}/g" "${STAGING_YAML}"
    else
        sed -i "s/<AWS_ACCOUNT_ID>/${AWS_ACCOUNT_ID}/g" "${STAGING_YAML}"
    fi
    echo "✅ staging.yaml 업데이트 완료: imageRegistry → ${ECR_REGISTRY}"
else
    echo "✅ staging.yaml: AWS Account ID 이미 설정됨"
fi

echo ""
echo "=============================================="
echo "  ✅ staging 클러스터 준비 완료!"
echo "=============================================="
echo ""
